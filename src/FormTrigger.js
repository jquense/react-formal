...
export default withState((messages, groups, form, submitting, props, ref) => {
  const {
    for: name,
    group,
    formKey,
  ...
  const names = name || namesForGroup(group, groups)

  const eventHandlers = {}
  for (const event of events) {
    eventHandlers[event] = (...args) => {
      let fieldsToValidate = triggers || names
      if (noValidate || !fieldsToValidate) return
      ...
      if (group === '@submit') return form.onSubmit()
      form.onValidate(fieldsToValidate, event, args)
    }
  }
