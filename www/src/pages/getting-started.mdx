...
React Formal, consolidates and tracks your form's state, input values are mapped
to a form value via their `name` and works very similar to how you might serialize
a native HTML form.
...
a field. The only expectation React Formal has it that fields accept a `value`
...
again. Once it passes an `onSubmit` callback will fire. After which React Formal
considers the submission finished and is ready to continue accepting input.
...
off the form data to the server.
