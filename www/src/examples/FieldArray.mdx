...
that error's and other metadata move with the data during list manipulations.
...
Generally `FieldArray`s will render some set of child `Field`s that handle their
