...
>
  <fieldset>
    ...
    <label>
      <Form.Field type="checkbox" name="colors" value="yellow" /> Yellow
    </label>
    <label>
      <Form.Field type="checkbox" name="colors" value="orange" /> Orange
    </label>
  </fieldset>
</Form>;
```
...
Validation events can be controlled finely via the [`events`](#events) prop. Events
control which handlers `<Field>` passes to the input it renders. Multiple event
handlers can be configured with an array of handler names.
...
<>
  <Form schema={schema}>
    <label>
      Validates onBlur
      <Form.Field name="name" events="onBlur" />
    </label>
  ...
  <Form schema={schema}>
    <label>
      ...
      <Form.Field name="name" events={["onBlur", "onChange"]} />
    </label>
    <Form.Message for="name" className="error" />
  </Form>
</>;
...
For more complex situations `events` accepts a function that is based the `meta` for the field
...
<Form schema={schema}>
  <label>
    Enter an Email
    <Form.Field name="email" events={onBlurThenChangeAndBlur} />
  </label>
...
As a convenience React Formal exports a few common event configurations
you can mix and match if that is helpful.

```jsx static
import { EventStrategies } from "react-formal";
...
<Form schema={schema}>
  <label>
    Email
    <Form.Field name="email" events={EventStrategies.onBlur} />
  </label>
  <Form.Message for="email" className="error" />
</Form>;
```
