...
function attachChildren(children, context, test) {
  ...
  function clone(child) {
    ...
    if (props.children) props = babelHelpers._extends({}, child.props, { children: attachChildren(props.children, context, test) });

    return new ReactElement(child.type, child.key, child.ref, child._owner, test(child.type) ? babelHelpers._extends({}, child._context, context) : child._context, props);
  }
}